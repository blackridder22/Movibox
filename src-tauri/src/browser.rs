...
const BROWSER_LABEL: &str = "movibox-browser";

const CHROME_INIT_SCRIPT: &str = r#"
(function () {
  if (window.__moviboxBrowserBooted) return;
  window.__moviboxBrowserBooted = true;
...
(function () {
  if (window.__moviboxCaptureBooted) return;
  window.__moviboxCaptureBooted = true;
  const isInstall = (u) => {
    try {
      const s = String(u);
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            builder = builder.on_navigation(move |url| {
                ...
                if url.scheme() == "stremio" || s.contains("/manifest.json") {
                    ...
                    let _ = nav_app.emit("movibox://browser-stremio-capture", s.to_string());
                    return false;
                }
                true
            });
