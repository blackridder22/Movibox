...
// Retain the status-menu implementation, but leave it inactive on macOS.
pub const STATUS_MENU_ENABLED: bool = !cfg!(target_os = "macos");
...
pub fn build(app: &AppHandle) -> tauri::Result<()> {
    let show = MenuItem::with_id(app, "tray_show", "Show MoviBox", true, None::<&str>)?;
    let themes: [(&str, &str); 13] = [
        ("cool-grey", "MoviBox default"),
        ("nord", "Nord"),
        ("stremio", "Stremio"),
        ("crunch", "Crunchy"),
        ("tokyo-night", "Royal"),
    ...
    let quit = MenuItem::with_id(app, "tray_quit", "Quit MoviBox", true, None::<&str>)?;
    let pause = MenuItem::with_id(app, "tray_pause", "Pause all downloads", true, None::<&str>)?;
    let resume = MenuItem::with_id(app, "tray_resume", "Resume downloads", true, None::<&str>)?;
    let settings = MenuItem::with_id(app, "tray_settings", "Settings", true, None::<&str>)?;
    let menu = Menu::with_items(app, &[&show, &pause, &resume, &settings, &sep, &quit])?;
    ...
    let mut builder = TrayIconBuilder::with_id("harbor-tray")
        ...
        .on_menu_event(|app, event| {
            ...
            match id {
                ...
                "tray_pause" => {
                    let _ = crate::acquisition::acquisition_pause_all(app.clone(), app.state());
                }
                "tray_resume" => {
                    let _ = crate::acquisition::acquisition_resume_all(app.clone(), app.state());
                }
                "tray_settings" => {
                    show_main(app);
                    let _ = app.emit("movibox://navigate", "settings");
                }
