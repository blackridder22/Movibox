...
static FLAG_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"[\x{1F1E6}-\x{1F1FF}][\x{1F1E6}-\x{1F1FF}]").unwrap());
...
static TORRENTIO_NOISE_PREFIX_RX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"^[\s\x{1F464}\x{1F465}\x{1F4BE}\x{1F4E6}\x{26A1}\x{1F310}\x{1F4FA}\x{1F3AC}\x{1F50A}]+",
    )
    .unwrap()
});
...
static ANIME_HASH_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\[([0-9A-F]{8})\]").unwrap());
...
static RD_UNCACHED_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\[RD(?:[\s\-]?download|\x{2B07}\x{FE0F}?|\x{23F3})\]").unwrap());
static TB_UNCACHED_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\[TB(?:[\s\-]?download|\x{2B07}\x{FE0F}?|\x{23F3})\]").unwrap());
static AD_UNCACHED_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\[AD(?:[\s\-]?download|\x{2B07}\x{FE0F}?|\x{23F3})\]").unwrap());
static PM_UNCACHED_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\[PM(?:[\s\-]?download|\x{2B07}\x{FE0F}?|\x{23F3})\]").unwrap());
static DL_UNCACHED_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\[DL(?:[\s\-]?download|\x{2B07}\x{FE0F}?|\x{23F3})\]").unwrap());
...
static STREAMFUSION_UNCACHED_SERVICE_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?im)^\x{2B07}\x{FE0F}?download\s*\n([^\n]+)").unwrap());
...
static BIT_DEPTH_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\b(8|10|12)\s*bit\b").unwrap());
static REPACK_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bREPACK(\d+)?\b").unwrap());
...
static FN_TORRENTIO_PREFIX_RX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)^(?:torrentio|comet|mediafusion|aiostreams|knightcrawler|jackettio|torbox)\b")
        .unwrap()
});
...
static FN_NOISE_LEAD_RX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"^[\x{1F464}\x{1F465}\x{1F4BE}\x{1F4E6}\x{26A1}\x{1F310}\x{1F4FA}\x{1F3AC}\x{1F50A}]",
    )
    .unwrap()
});
static FN_LABEL_PREFIX_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)^(?:size|seeders?|peers?|languages?)\s*[:=]").unwrap());
static FN_LIBRARY_LABEL_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)^\[(?:RD|TB|AD|PM|DL)\+\]\s+\S+\s+library").unwrap());
...
static FN_EPISODE_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bS\d{1,2}E\d{1,3}\b").unwrap());
...
static FN_CODEC_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(?:x264|x265|HEVC|AVC|h264|h265|AV1|MPEG2|MPEG-2)\b").unwrap());
static FN_CONTAINER_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\.(?:mkv|mp4|m4v|avi|ts)\b").unwrap());

static SEASON_PACK_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(complete|season[\s\.]?pack|s\d{1,2})\b").unwrap());
static SEASON_PACK_BARE_S_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bs\d{1,2}E").unwrap());

static EPTITLE_TRIM_LEAD_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"^[\.\-_\s]+").unwrap());
...
static INVISIBLE_RX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"[\x{200B}-\x{200D}\x{2060}\x{FEFF}]").unwrap());
static VARIATION_SELECTOR_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"\x{FE0F}").unwrap());

static COMET_BINGE_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)^comet\|([a-z\-]+)\|").unwrap());
...
static MF_TRB_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bTRB\b|\bTorBox\b").unwrap());
static MF_TB_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bTB\b").unwrap());
static MF_RD_FULL_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bReal[\s\-]?Debrid\b").unwrap());
...
static MF_DL_FULL_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bDebrid[\s\-]?Link\b").unwrap());
...
fn ptt_parse(title: &str) -> PttResult {
    ...
    static PTT_YEAR_RX: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"[^a-zA-Z0-9][\(\[]?((?:19[0-9]|20[012])[0-9])[\)\]]?").unwrap());
    ...
    static PTT_RES_NUM_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)([0-9]{3,4}[pi])").unwrap());
    static PTT_RES_4K_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\b(4k)").unwrap());
    static PTT_RES_1080_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)FHD|\b1080\b").unwrap());
    ...
    static PTT_OPENMATTE_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)OPEN[. ]MATTE").unwrap());
    if let Some(m) = PTT_OPENMATTE_RX.find(title) {
        r.openmatte = true;
        note_idx!(Some(m.start()));
    }

    static PTT_HARDCODED_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"HC|HARDCODED").unwrap());
    ...
    static PTT_REPACK_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)REPACK|RERIP").unwrap());
    ...
    static PTT_CRITERION_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"\bCriterion\b").unwrap());
    ...
    static PTT_CODEC_264_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)h[-. ]?264|avc").unwrap());
    static PTT_CODEC_OTHER_RX: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"(?i)dvix|mpeg2|divx|xvid|x[-. ]?26[45]").unwrap());
    ...
    static PTT_EPISODE_EP_RX: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"(?i)[e\u{00E9}]p(?:isode)?[. _-]?([0-9]{1,5})").unwrap());
...
fn parse_languages(text: &str) -> Vec<String> {
    ...
    for m in FLAG_RX.find_iter(text) {
        ...
        let code: String = vec![ca as u8 as char, cb as u8 as char]
            .into_iter()
            .collect();
...
fn parse_container(
    ...
    for src in [filename_hint, Some(filename_line), Some(text)]
        .into_iter()
        .flatten()
    {
        if src.is_empty() {
...
fn parse_cache_flags(
    ...
    let mark_uncached =
        |slug: &str, out: &mut BTreeMap<String, bool>, denied: &mut HashSet<String>| {
...
fn addon_name_slug(name: Option<&str>) -> Option<&'static str> {
    ...
    static A_RD_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"real[\s\-]?debrid|\brd\b").unwrap());
    static A_AD_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"all[\s\-]?debrid|\bad\b").unwrap());
    static A_PM_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"premiumize|\bpm\b").unwrap());
    static A_DL_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"debrid[\s\-]?link|\bdl\b").unwrap());
...
fn parse_edition(text: &str, ptt: &PttResult) -> Option<String> {
    ...
    static ED_NORMALIZE_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"[\.\s]+").unwrap());
...
mod tests {
    ...
    fn parses_yts_1080p_x265() {
        let s = mk_stream("The.Dark.Knight.2008.1080p.BluRay.x265-YTS", "YTS");
        let p = parse_stream(s);
        assert_eq!(p.resolution, Resolution::P1080);
    ...
    fn parses_cam_release_lowest_quality() {
        let s = mk_stream("Some.Movie.2024.HDCAM.x264-NEW\n💾 1.2 GB", "Torrentio");
        let p = parse_stream(s);
