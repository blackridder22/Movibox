...
static STRONG_ADDON_RX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)mediafusion|comet").unwrap());
...
fn is_webish_source(s: Source) -> bool {
    matches!(
        s,
        Source::WebDl | Source::WEBRip | Source::BluRay | Source::BDRip
    )
}
...
pub fn compute_corpus_stats(streams: &[ParsedStream], opts: &ScoreOptions) -> CorpusStats {
    ...
    let is_tracked = |s: &ParsedStream| -> bool {
        opts.active_debrids
            ...
            || s.seeders
                .map(|sd| sd >= TRACKING_MIN_SEEDERS)
                .unwrap_or(false)
    };
    ...
    let theater = tracked
        .iter()
        .filter(|s| is_theater_source(s.source))
        .count();
    let webish = tracked
        .iter()
        .filter(|s| is_webish_source(s.source))
        .count();
    ...
    if let (Some(size), Some(runtime)) = (s.size, opts.runtime_minutes) {
        if size > 0 && runtime > 0 {
            ...
            if required > budget * 1.1 {
                let sev = if required > budget * 1.5 {
                    -120.0
                } else {
                    -45.0
                };
...
pub fn score_stream(
    ...
    if !preferred.is_empty() {
        if parsed.audio_languages.is_empty() {
            ...
            let is_multi = parsed.audio_languages.iter().any(|l| l == "Multi");
    ...
    } else if opts.prefer_single_audio_track && parsed.audio_languages.iter().any(|l| l == "Multi")
    {
        score -= 12.0;
        reasons.push(ScoreReason {
            signal: "html5-multi-audio-penalty".to_string(),
...
pub fn rank_and_pick(
    ...
    cached_first.sort_by(|a, b| {
        let ac = if is_cached_scored(a, active_debrids) {
            1
        } else {
            0
        };
        let bc = if is_cached_scored(b, active_debrids) {
            1
        } else {
            0
        };
    ...
    let primary = picked
        .or_else(|| {
            cached_first
                .iter()
                .find(|s| !is_theater_source(s.parsed.source))
        })
...
mod tests {
    ...
    fn scoring_addon_priority_bonus_decays_with_position() {
        ...
        assert_eq!(
            score_stream(none, &empty_opts(), &empty_corpus()).score,
            20.0
        );
    }
