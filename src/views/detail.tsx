...
import { Check, Download, HardDrive, Layers, Pencil, Plus, RotateCcw, Star } from "lucide-react";
...
export function DetailView({
  ...
  const startDownload = useCallback(() => {
    const launch = (episode: PlayEpisode | undefined) =>
      openPicker(playMeta, episode, { intent: "download" });
    if (!isSeries) {
      launch(undefined);
      return;
    }
  ...
  }, [isSeries, isAnime, animeEpisodes, lastPlay, openPicker, playMeta]);
  const downloadLabel =
    isSeries && lastPlay
      ? t("Download S{s}:E{e}", { s: lastPlay.season, e: lastPlay.episode })
      : t("Download");
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 px-12 pb-14">
            <div className={awardsInDescription ? "max-w-3xl" : undefined}>
              ...
              >
                {upcoming ? (
                  <UpcomingCta detail={detail} onTry={startDownload} />
                ) : (
                  <button
