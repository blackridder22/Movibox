...
const SECTION_META: Partial<Record<SectionId, { label: string; sub: string }>> = {
  basics: {
    label: "Get started",
    sub: "Connect Stremio, choose download sources, and pick your look.",
  },
  account: {
    label: "Account",
    sub: "Your Stremio sign-in. Library, collections, and addons sync from here.",
  },
  library: {
  ...
  trakt: {
    label: "Trakt",
    sub: "Connect your Trakt account to sync your watchlist and pull personalized recommendations.",
  },
  anilist: {
    label: "AniList",
  ...
  mal: {
    label: "MyAnimeList",
    sub: "Connect your MyAnimeList account and browse your list.",
  },
  simkl: {
    label: "Simkl",
    sub: "Connect your Simkl account and sync your plan-to-watch list across apps.",
  },
  letterboxd: {
    label: "Letterboxd",
    sub: "Bring your Letterboxd watchlist, diary, liked films and lists into MoviBox via the Stremboxd bridge.",
  },
  streaming: {
    label: "Streaming sources",
    sub: "How MoviBox finds and resolves downloadable files. Debrid keys and addon installs live here.",
  },
  streamFilters: {
  ...
  p2p: {
    label: "P2P & servers",
    sub: "MoviBox's built-in torrent acquisition engine, its self-test, and compatible source servers.",
  },
  language: {
    label: "Languages",
  ...
  hotkeys: {
    label: "Hotkeys",
    sub: "Every navigation shortcut MoviBox responds to. Click a binding to rebind it.",
  },
  theme: {
    label: "Theme & appearance",
    sub: "Color presets, custom backgrounds, and the font pair MoviBox renders in.",
  },
  webhooks: {
    label: "Webhooks",
  ...
  bug: {
    label: "Report a bug",
    sub: "Collect diagnostics for a MoviBox issue. Screenshots and screen recordings welcome.",
  },
  advanced: {
    label: "Advanced",
...
export function Settings() {
  ...
  const [active, setActive] = useState<SectionId>(
    normalizeSection(settingsSectionRequest.section as SectionId | null),
  );
  ...
  useEffect(() => {
    if (settingsSectionRequest.section) {
      setActive(normalizeSection(settingsSectionRequest.section as SectionId));
    }
          ...
          <div data-tauri-drag-region className="mx-auto flex max-w-3xl flex-col gap-10 px-12">
            <header className="flex flex-col gap-2">
              <h1 className="font-display text-[44px] font-medium leading-[1.05] tracking-tight text-ink">
                {t((SECTION_META[active] ?? SECTION_META.streaming!).label)}
              </h1>
              <p className="text-[15px] text-ink-muted">
                {t((SECTION_META[active] ?? SECTION_META.streaming!).sub)}
              </p>
            </header>
...
const REMOVED_PLAYER_SECTIONS = new Set<SectionId>([
  "relay",
  "player",
  "mpv",
  "anime",
...
function normalizeSection(section: SectionId | null): SectionId {
  if (!section || REMOVED_PLAYER_SECTIONS.has(section)) return section ? "streaming" : "account";
  return section;
}
