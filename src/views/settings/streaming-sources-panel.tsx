  ...
  return (
    <>
      <Section
        ...
        subtitle={t(
          "How aggressively MoviBox rejects shady or mismatched sources before showing them in the download picker.",
        )}
      >
        <StreamFilterPicker
      ...
      <Section
        ...
        subtitle={t(
          "MoviBox ranking puts the best-scoring sources first. Add-on order follows your priority and preserves each add-on's result order.",
        )}
      >
        ...
        <p className="mt-3 rounded-xl border border-edge-soft bg-canvas/40 px-4 py-3 text-[12.5px] leading-relaxed text-ink-muted">
          {t(
            "Using AIOStreams or another aggregator add-on? It filters first, then MoviBox applies the safety and result-order settings above. If results look too thin, keep one side permissive.",
          )}
        </p>
      </Section>
  ...
  const options: Array<{ id: "condensed" | "stremio"; label: string; sub: string }> = [
    {
      ...
      label: t("Condensed"),
      sub: t(
        "Default. Top pick first, quality tiles, and an All-Sources drawer. MoviBox scores and ranks results.",
      ),
    },
    {
      id: "stremio",
...
function StreamSortPicker({
  ...
  const options: Array<{ id: "harbor" | "addon"; label: string; sub: string }> = [
    {
      id: "harbor",
      label: t("MoviBox ranking"),
      sub: t(
        "Default. MoviBox parses and scores every source and surfaces the best quality first.",
      ),
    },
    {
      id: "addon",
