...
export function AccountStub() {
  ...
  const displayName =
    activeProfile?.name || user?.fullname || user?.email.split("@")[0] || t("MoviBox user");
  ...
  return (
    <div className="flex flex-col gap-5">
      <Section
        title={t("MoviBox identity")}
        subtitle={t("Your local profile, layered on top of your Stremio account.")}
      >
      ...
      <Section
        title={t("Profiles")}
        subtitle={t(
          "Everyone who uses this MoviBox gets a separate profile, avatar, color, and optional PIN.",
        )}
      >
      ...
      <Section
        ...
        subtitle={t("Library, collections, and addon configuration sync from this account.")}
      >
        {user ? (
          ...
          <div className="flex items-center justify-between gap-4 rounded-2xl border border-edge-soft bg-canvas/40 p-5">
            <div className="flex flex-col">
              ...
              <span className="text-[12.5px] text-ink-subtle">
                {t("Sign in to sync your library, collections, and addons.")}
              </span>
            </div>
            <button
              onClick={() => setShowAuth(true)}
      ...
      <Section
        ...
        subtitle={t(
          "MoviBox pulls your addon collection from Stremio. Manage individual addons in Download sources.",
        )}
      >
        <SyncedAddonsCard />
      </Section>
