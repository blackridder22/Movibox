  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        Icon: IconLanguages,
        keywords: ["audio", "preferred", "tracks", "source ranking"],
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        label: "Hotkeys",
        Icon: IconHotkeys,
        keywords: ["shortcuts", "keys", "keyboard", "bindings", "navigation"],
      },
    ],
  },
  {
...
const REMOVED_PLAYER_SEARCH =
  /\b(player|playback|mpv|libmpv|aspect ratio|seek bar|subtitle style|skip intro|next episode prompt|anime4k|smooth motion|frame interpolation|picture quality|hardware acceleration|tone-mapping|downmix|discord rich presence|watch party|player log|pause when minimized|pause when unfocused|streaming server|whole file while streaming|finish watching|direct torrent streaming|p2p status overlay)\b/i;

function isRemovedPlayerOption(option: SettingsOption): boolean {
  return REMOVED_PLAYER_SEARCH.test(
    [option.label, option.anchorTitle, ...(option.keywords ?? [])].filter(Boolean).join(" "),
  );
}
...
export function SettingsNav({
  ...
  const optionMatches = useMemo<SettingsOption[] | null>(() => {
    ...
    return SETTINGS_OPTIONS.filter(
      (o) =>
        !REMOVED_PLAYER_SECTIONS.has(o.section) &&
        !isRemovedPlayerOption(o) &&
