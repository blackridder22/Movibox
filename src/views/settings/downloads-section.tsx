...
export function DownloadsSection() {
  ...
  useEffect(() => {
    ...
    downloadDir()
      .then((directory) => {
        if (!cancelled) setSystemDefault(directory);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const isCustom = Boolean(settings.downloadDir);
  ...
  const pickFolder = async () => {
    try {
      ...
      // The native picker was dismissed or is unavailable in a web preview.
    }
  };
  ...
  const revealCurrent = async () => {
    ...
    try {
      ...
      // Revealing a folder is a convenience and should never break settings.
    }
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      <ToggleRow
        label="Create folders for movies and series"
        note="Organize files by title, season, and episode"
        value={settings.downloadCreateFolders}
        onChange={(value) => update({ downloadCreateFolders: value })}
      />
      <div className="flex items-center justify-between gap-3 rounded-xl border border-edge-soft bg-canvas/40 px-4 py-3">
        <div className="flex min-w-0 flex-col gap-0.5">
          <span className="text-[11.5px] font-semibold uppercase tracking-[0.16em] text-ink-subtle">
            {isCustom ? "Custom location" : "System Downloads folder"}
          </span>
