...
import { SeriesDownloadPanel } from "./series-download-panel";
...
export function AnimeEpisodes({
  ...
  const downloadSeasons = useMemo(
    () =>
      Array.from(
        new Set(episodes.map((episode) => episode.imdbSeason ?? episode.seasonNumber ?? 1)),
      )
  ...
  return (
    <div data-anime-episodes className="flex flex-col gap-6 scroll-mt-24">
      ...
      {!isOneOff && <SeriesDownloadPanel meta={meta} seasons={downloadSeasons} />}
      {isOneOff ? (
