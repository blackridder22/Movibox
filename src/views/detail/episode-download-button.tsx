import { ArrowDownToLine, Check, Clock3, Play, RotateCw, X } from "lucide-react";
...
export function EpisodeDownloadButton({
  ...
  const downloading = status === "queued" || status === "downloading" || status === "canceling";
  const scheduled = status === "scheduled";
  const paused = status === "paused";
  const done = status === "done";
  const failed = status === "error" || status === "needsResolution" || status === "interrupted";
  const persistent = downloading || scheduled || paused || done || failed;
  ...
  return (
    <button
      ...
      aria-label={
        scheduled
          ? t("Download scheduled, click to cancel")
          : paused
            ? t("Download paused, click to resume")
            : downloading
    ...
    >
      {scheduled ? (
        <Clock3 size={dim * 0.42} strokeWidth={2.2} />
      ) : paused ? (
        <Play size={dim * 0.42} strokeWidth={2.2} fill="currentColor" />
      ) : downloading ? (
        <>
          <svg
            width={dim}
