...
export const ENGINE_WARMING_MESSAGE =
  "MoviBox's peer-to-peer engine is warming up. This clears on its own in a few seconds, then Download works.";
...
export function humanError(code: string): string {
  switch (code) {
    ...
    case "external-url-only":
      return "This source only opens in an external browser and cannot be downloaded by MoviBox.";
    case "youtube-only":
