  ...
  return (
    <>
      {addonsSettled && (!streamIds || streamIds.length === 0) && (
        <EmptyState
          message="MoviBox couldn't resolve a usable ID for this title. Add a TMDB key in Library settings or sign in to Stremio to broaden coverage."
      ...
      {addonsSettled &&
        ...
        allCount === 0 && (
          <EmptyState
            message="No downloadable sources turned up, and no debrid is configured. Real-Debrid, TorBox, AllDebrid, Premiumize, or Debrid-Link can unlock raw torrent results. Some add-ons include debrid resolution themselves."
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-300/30 bg-amber-300/[0.06] px-5 py-3.5 text-[13px] text-ink">
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            <p className="text-[12.5px] leading-snug text-ink-muted">
              {allCount === 1
                ? "Clean releases for this title haven't surfaced yet. The result below may not match the title you're looking for, so confirm the filename and size before downloading."
                : "Clean releases for this title are still scarce. Confirm the filename and size before downloading."}
              {rawCount - allCount > 0 && !forceShowAll
                ? ` MoviBox dropped ${rawCount - allCount} suspicious or mismatched result${rawCount - allCount === 1 ? "" : "s"}.`
                : ""}
            </p>
          </div>
          {rawCount - allCount > 0 && !forceShowAll && (
