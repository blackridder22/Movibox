...
export async function startDeepLinkBridge(): Promise<() => void> {
  ...
  try {
    ...
    const unlistenNative = makeSafeTauriUnlisten(
      await listen<string>("movibox:stremio-deeplink", (e) => {
        const u = e.payload;
    ...
    const unlistenBrowserCap = makeSafeTauriUnlisten(
      await listen<string>("movibox://browser-stremio-capture", forwardLinuxBrowserInstall),
    );
    try {
    ...
    console.warn("[movibox] deep-link bridge failed", e);
    return () => {};
  }
}
