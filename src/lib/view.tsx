...
export function ViewProvider({ children }: { children: ReactNode }) {
  ...
  const openPlayer = useCallback((src: PlayerSrc) => {
    if (!src.url) return;
    if (src.meta.id.startsWith("local:") || !/^https?:\/\//i.test(src.url)) {
      void import("@tauri-apps/plugin-opener")
        .then(({ openPath }) => openPath(src.url))
        .catch((error) => console.warn("[movibox] could not open local file externally", error));
      return;
    }
    if (src.isLive || src.meta.id.startsWith("iptv:")) {
      console.warn("[movibox] live streams cannot be downloaded as ordinary files");
      return;
    }
    void import("@/lib/download/downloads-store")
      .then(({ enqueueDownload }) =>
        enqueueDownload({
          meta: src.meta,
          episode: src.episode,
          streamLabel: src.streamRef?.quality ?? src.streamRef?.resolution ?? "Direct source",
          provider: "direct",
          infoHash: src.streamRef?.infoHash ?? null,
          fileIndex: src.streamRef?.fileIdx ?? null,
          url: src.url,
          headers: src.headers ?? null,
        }),
      )
      .catch((error) => console.warn("[movibox] could not queue resolved source", error));
  }, []);
  ...
  const replacePlayerSrc = openPlayer;
